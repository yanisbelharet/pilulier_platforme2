...
export default function LandingPageV3({ config, onPurchase }: { config: any, onPurchase: (p: number, product: any, formData?: any) => void }) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      let pastFirstButton = false;

      if (firstButton) {
        pastFirstButton = firstButton.getBoundingClientRect().bottom < 0;
      } else {
        pastFirstButton = window.scrollY > 400;
      }
      
      setShowStickyButton(pastFirstButton);
    };
